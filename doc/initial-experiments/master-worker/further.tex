...
registered with \ccode{reg-finalize} to run a cleanup on the hosts of
...
takes the form of a \ccode{read-distributed-csv} call that forwards on
most of the arguments to server-side \ccode{read-csv} functions. The
...
\ccode{ops} defined. Operations between vectors are forwarded on to the
...
\ccode{head} function as well as simple surjective functions.
...
user-side master \R session. As an example, the \ccode{unique} function
attains the unique elements within an array; it is implemented in
MapReduce form of \ccode{unique} operations within each node, followed
by a \ccode{unique} over their combined output within the master node.
...
To illustrate: a host containing elements \ccode{100} to \ccode{150}
will have the example selection \ccode{100-130} translated to
\ccode{1-30}, as these are the equivalent elements relative to that
specific node.

\subsubsection{Description of System Changes}\label{description-of-system-changes}
...
\ccode{lapply}. This has been replaced with a semi-parallel function
...
declarative and more R-like paradigm. Concretely, \ccode{send} and
\ccode{receive} have been replaced with \ccode{as-distributed} and the
empty subset function (\ccode{empty-subset}) respectively. This is matched
...
with operations and reductive functions such as \ccode{table} run upon
it.
...
with a \pkg{future}-like \ccode{resolve} attribute indicating this.
...
related operations such as checking \ccode{is-na} and
\ccode{all-equal}. When interacting with distributed objects, it is not
entirely clear how this should be performed. For example, the class of
every distributed vector is \ccode{distributed-vector}. This is the
case regardless of the class of the underlying chunks that make it up.
Even if the chunks are all logical, a call to \ccode{is-logical} will
...
a parent \ccode{distributed-vector} class.
...
\ccode{table}. With profiling and optimisation this
