...
The prior section was necessarily largely
speculative, considerate of a number of potential architectural choices,
with relatively equal weighting;  
more concrete evaluation can be produced,
and alternatives can be compared against what has been explicitly
observed to work and not work. In addition, several new and prior unconsidered aspects
...
akin to the \pkg{SNOW} cluster object, serves as a reference with which
to declare distributed objects upon. Distributed objects exist
conceptually as the actual data split across \pkg{RServe} instances, and a
...
registered with \code{reg-finalize} to run a cleanup on the hosts of
...
their associated \pkg{RServe} instances, described in a supplied cluster
...
takes the form of a \code{read-distributed-csv} call that forwards on
most of the arguments to server-side \code{read-csv} functions. The
...
\code{ops} defined. Operations between vectors are forwarded on to the
...
relevant elements to exist on the same \pkg{RServe} instance. This creates a
...
\code{head} function as well as simple surjective functions.
...
user-side master \R{} session. As an example, the \code{unique} function
attains the unique elements within an array; it is implemented in
MapReduce form of \code{unique} operations within each node, followed
by a \code{unique} over their combined output within the master node.
...
To illustrate: a host containing elements \code{100} to \code{150}
will have the example selection \code{100-130} translated to
\code{1-30}, as these are the equivalent elements relative to that
specific node.
...
extensive. Beyond the obvious additions alluded to 
 that weren't previously present, some more notable
...
\code{lapply}. This has been replaced with a semi-parallel function
...
these operations, issues still remain.
...
declarative and more R-like paradigm. Concretely, \code{send} and
\code{receive} have been replaced with \code{as-distributed} and the
empty subset function (\code{empty-subset}) respectively. This is matched
...
with operations and reductive functions such as \code{table} run upon
it.
...
with a \pkg{future}-like \code{resolve} attribute indicating this.
...
related operations such as checking \code{is-na} and
\code{all-equal}. When interacting with distributed objects, it is not
entirely clear how this should be performed. For example, the class of
every distributed vector is \code{distributed-vector}. This is the
case regardless of the class of the underlying chunks that make it up.
Even if the chunks are all logical, a call to \code{is-logical} will
...
a parent \code{distributed-vector} class.

The nature of the connections to \pkg{RServe} instances have proven
...
\pkg{RServe} form of connection. In the mean time, an abstraction layer for
...
\code{table}. With profiling and optimisation this
