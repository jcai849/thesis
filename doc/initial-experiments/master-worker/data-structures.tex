...
Using the nectar cluster, the \ccode[sh]{hdp} node was used as a master
with which to control worker nodes \ccode[sh]{hadoop1} through to
\ccode[sh]{hadoop8}. \pkg{RServe} was used as the means for control and
communication with the workers. S3 classes were defined for
\ccode{cluster}, \ccode{node}, \ccode{distributed-object} and
\ccode{distributed-vector}. Communication functions operate serially,
but were written with future parallelisation and speed in mind.

The \ccode{node} class contains information on connections to the
worker nodes. The \ccode{cluster} class is a collection of
\ccode{node}s. The cluster is set up using \ccode{make-cluster},
which \ccode[sh]{ssh} into the hosts and launches RServe, along with the
relevant libraries and functions. The global environment local to
specific nodes can be checked with the \ccode{peek} method, serving
purely as a sanity check at present. \ccode{send} is a generic with
methods defined for the \ccode{node} and \ccode{cluster} classes; it
...
referenced by the \ccode{cluster} or \ccode{node} objects. It can
equally handle objects with smaller \ccode{split} sizes than there are
nodes, dispersing them maximally. \ccode{send} is used just to get
data to the nodes to bootstrap the system, and wouldn't be used by the
end-user.

\ccode{distributed-object} at present has no methods defined, serving
as a placeholder for an abstract distributed class.
\ccode{distributed-vector} inherits from \ccode{distributed-object},
...
typically being a UUID generated with the \ccode{distributed-vector}
creation.

\ccode{receive} is the complement to \ccode{send}, giving a
\ccode{distributed-object} as an argument, and receiving the unsplit
referent of the \ccode{distributed-object} as the value. The method
will have additional usage as a remote version, which would enable
point-to-point communication through a node calling \ccode{receive} on
...
As a means of testing operations between \ccode{distributed-vector}
objects, S3 \ccode{ops} methods were defined, using a complex quoting
function in order to call the correct \ccode{generic} and reference
...
No quality-of-life methods such as \ccode{print} were defined, with
...
\ccode{abc}, what occurs is that on every node
\ccode{a} and \ccode{b} exist on, they are summed together, with the
result saved as a new variable with a UUID name; a reference to the name
and locations is then stored locally in the variable \ccode{c}. Were
\ccode{a} and \ccode{b} not to be assigned, however, the result would
...
\ccode{distributed-vector} as well as a generalisation to vectors of
...
some kind of \ccode{align} method, which as discussed in the previous
...
the right methods defined such that \ccode{summary} and the like work
smoothly. This would lead naturally to the definition of
\ccode{distributed-data-frame} objects and the like. Furthermore, a
