...
which \code[sh]{ssh} into the hosts and launches \pkg{RServe}, along with the
...
due to difficulties with point-to-point communication using \pkg{RServe}.
...
communication is not so easy to perform in reality, as \pkg{RServe} forks a
...
so easily permitted through (ab)using \pkg{RServe}. In turn, some custom
