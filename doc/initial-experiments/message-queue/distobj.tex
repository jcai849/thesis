...
\ccode{adj}, wherein vectors are operated upon in
adjacently.
...
An example is given in the expression \ccode{recycle}, where
...
\fig{distobj-eval-align}{Process of evaluation and alignment for distributed object references resulting from the expresssion, \ccode{do-call-distobj}}

Similarly to chunk manipulation, the central function for performing operations
is the \ccode{do-call} function, with a method defined for
\ccode{distobjref}. It possesses similar formal parameters to the standard
\ccode{do-call}, being \ccode{what}, \ccode{args}, and
\ccode{assign}, which respectively describe the function to be called, a list
of arguments to serve as arguments to \ccode{what}, and whether to return a
value or to save the result and monitor it as a distributed object.

A node acting in the role of client will call a
\ccode{do-call-distobj} on a list of distributed object
...
\ccode{find-target}.
From this target, \ccode{do-call-chunkref} is called with an
additional \ccode{target} argument relating the target chunk, on all of the
...
\ccode{do-call-msg} is called on the popped message, taking the
same arguments as \ccode{do-call-chunkref}.
Within \ccode{do-call-msg}, \ccode{reftorec}
and \ccode{alignment} are called to replace the reference
objects in the \ccode{args} list with copies of their referent objects at
indices analogous to the target chunk, thereby ensuring correctness in
adjacent operations between arrays.

The standard \ccode{do-call} is then used on the list of whole,
...
based on the resultant chunk's size after \ccode{do-call-msg}.
...
Therefore the distributed object references forming the list of \ccode{args}
in \ccode{do-call-distobj} can either have all of the index
...
completing the evaluation of the current \ccode{do-call-msg}. 
...
a singular node \(S\).
Let this object have reference \ccode{x} with full resolution on a node \(K\).
(\textcolor{dark2-1}{a}) The node \(K\) may execute the expression 
\ccode{do-call-distobj-assign}, returning an
...
given that there is only one distributed object reference in \ccode{args}, are
...
\ccode{do-call-msg-expr},
where \ccode{obj-place} refers to the value of \ccode{obj}.
(\textcolor{dark2-1}{e}) The expression will in turn call \ccode{reftorec} and
\ccode{alignment} on \ccode{Cm} and the \ccode{args} list,
which, given the resolved state of all chunks, will replace the
\ccode{distobj-place} in \ccode{args} with it's corresponding emerged
object, in this case the referent of \ccode{Cm}.
Upon completion of performing \ccode{f1} on the new \ccode{args} list, the
...
\ccode{do-call-distobj-list}, and
\ccode{x} is given as the target, then (\textcolor{dark2-2}{j}) with node \(S\) popping from queue
\(c_m\) once again, (\textcolor{dark2-2}{k}) the following expression is then formed on \(S\):
\ccode{do-call-msg-list}.
(\textcolor{dark2-2}{l}) As before, the expression will call \ccode{reftorec} and
\ccode{alignment} on \ccode{Cm} and the \ccode{args} list,
but the relevant index information on the chunks belonging to the value of
distributed object reference \ccode{y} is missing, and will be blocked
indefinitely, as there is no means of determining where in the distributed
object \(y\) is the chunk adjacent to chunk \ccode{Cm}.
...
a \ccode{do-call-distobj} until full chunk resolution of the
...
If the \ccode{do-call-msg} was made concurrent through a
...
\ccode{do-call-msg} and it's surrounding constructs implies a
parallel operation such as that provided by the \ccode{parallel} package,
located principally in the \ccode{mcparallel} function.
An initial problem with reliance on \ccode{mcparallel} is the
