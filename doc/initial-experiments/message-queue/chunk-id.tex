...
where variables \ccode{x}, \ccode{y}, and \ccode{z} are chunk references,
and variables \ccode{i} and \ccode{j} are local. 
...
popped from.
Without loss of generality, the function \ccode{f} is considered to take
...
A diagram of the server-originated chunk ID model is given in figure \cref{fig:server-chunk-id}.

\fig{server-chunk-id}{Space-Time Diagram of Server-Originated Chunk ID}

An important aspect of figure \cref{fig:server-chunk-id} is the fact that the client has to
wait to get the chunk ID's of the chunk references \ccode{y} and \ccode{z}
...
modelled also has issue with errors; consider if the \ccode{assign-x-y} had
been faulty, with the resultant operation of \ccode{f-c1} rendering an error.
This would not be determined by the client untile the completion of
\ccode{f-z}, in which an error would presumably result.
Worse, if the chunk reference \ccode{x} was given as an additional argument to
another server, which in turn requested the chunk \ccode{C1} from the node
\ccode{C1} resided upon, the error would propagate, with the source of the
...
which have a state of \ccode{resolved} or \ccode{unresolved}, with failures
...
errors.
The introduction of future terminology of \ccode{resolved-}, as well as
...
but the use of \ccode{resolved-} and it's associated barrier procedure,
\ccode{resolve} will enable the forcing of a greater degree of synchrony,
and allow tracing of errors back to their source in a more reliable manner.

A diagram of the proposed model, along with an additional \ccode{resolved-x}
