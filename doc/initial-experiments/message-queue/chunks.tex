...
order function \ccode{distributed-do-call}, which took functions as arguments
...
To this end, the client node has a function defined as \ccode{do-fun-at}, which takes in any function, and the ID of a chunk to perform the
function on.
An implementation is given by \cref{lst:vr-client}.
\ccode{do-fun-at} first composes a message to send to the chunk's queue, being a
...
Upon a message coming through, it runs a \ccode{do-fun} function on the
message, which in turn runs the function on the chunk named in the message. 
An implementation is given by listing \cref{lst:vr-server}.
...
providing an \ccode{INCR} operation, which can be used to generate a new chunk
...
For instance, the result may be an unexpected \ccode{NULL}, zero-length
...
chunk is largely encapsulated in a new function, \ccode{assign-fun-at}, as
...
For an operation of \ccode{DOFUN}, all that is run is a \ccode{do-call} on
the function and chunk specified, with a message being returned to the
client with the value of the \ccode{do-call}.
An operation of \ccode{ASSIGN} runs the same as \ccode{DOFUN}, with the
...
diagram, given by figure \cref{fig:chunk-assign}, following a Lamport form of
...
\fig{chunk-assign.tex}{Logical time diagram of chunk assignment for server-originated chunk ID}
