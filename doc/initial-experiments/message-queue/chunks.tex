...
order function \code{distributed-do-call}, which took functions as arguments
...
To this end, the client node has a function defined as \code{do-fun-at}, which takes in any function, and the ID of a chunk to perform the
function on.
...
Upon a message coming through, it runs a \code{do-fun} function on the
...
providing an \code{INCR} operation, which can be used to generate a new chunk
...
For instance, the result may be an unexpected \code{NULL}, zero-length
...
chunk is largely encapsulated in a new function, \code{assign-fun-at}, as
...
For an operation of \code{DOFUN}, all that is run is a \code{do-call} on
the function and chunk specified, with a message being returned to the
client with the value of the \code{do-call}.
An operation of \code{ASSIGN} runs the same as \code{DOFUN}, with the
