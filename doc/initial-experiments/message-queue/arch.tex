...
\ccode{do-call-chunkref}, alongside some function, in order to begin the
...
\ccode{do-call-chunkref} composes a message based on the request, pushing that
...
It pops the message off the related queue, and has \ccode{do-call-chunk}
...
The fields belonging to a \ccode{chunkRef} object are the following:
\begin{description}
...
Messages all belong to the \ccode{msg} class, and are currently categorised as
...
\tab{chunk-comm} {Communication between a client and server}
