...
Following from the implementation of the GLM given in Chapter \ref{sec:glm}, a description is provided here of another model, using a decidedly different and straightforward process.
Here, a Linear Regression model is defined, using the operators provided by \lsr, as detailed in Section \ref{sec:api-high}
...
is fitted using \pkg{biglm} as part of a demonstrative non-distributed \code{Reduce()} in listing \ref{lst:lm-reduce}.
...
\end{minted}
\caption{Splitting the iris dataframe into 15 chunks stored as elements of a list and reducing over the list with the biglm update function.}
\label{lst:lm-reduce}
\end{listing}
...
This uses a \code{do.ccall} function to create the initial biglm linear model object, which is then passed to the \code{dreduce()} function.
