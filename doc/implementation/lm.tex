...
Using the \textit{taxicab} dataset as an example, a linear model of the form,

\eq{taxicab-form}

is fitted using \pkg{biglm} as part of a demonstrative non-distributed \code{Reduce} in \cref{lst:lm-reduce}.

\src{lm-reduce}{Splitting the taxicab dataframe into 15 chunks stored as elements of a list and reducing over the list with the \pkg{biglm} update function.}
