...
Consider the highest level first: \pkg{largescalemodelr}.
...
Load the data with some function provided by \lsr to read distributed data in as a distributed object, with some \code{read.d*()} function.
\lsr provides the \code{read.dcsv()} function to read \texttt{csv} files distributed across various hosts and filesystems.
This may be stored as in:

\code{ddf <- read.dcsv(c("host1:/filepath/to/file1.csv", "host2:/filepath/to/file2.csv", "host3:/filepath/to/file3.csv")) }

Following this, a model function may be run over the distributed object; with a GLM as an example:

\code{mod.glm <- dglm(formula, ddf)}
...
Actual implementation of a custom model is given in the below Section \ref{glm}.
