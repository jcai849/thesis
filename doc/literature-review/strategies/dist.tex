...
computing over a simple network of computers. The general architecture
...
\code{crossprod} instead of \code{crossprod-manual}).
...
distributed computations is the \pkg{pbdMPI} package, which presents a
...
through the \code{ddmatrix} class, though the project offers some
support for other matrices. The \code{ddmatrix} class has nearly all
...
to distributed would be to prefix a \code{dd} to every \code{matrix} call, and bracket the program with a template as per listing
\cref{lst:bracket}.
...
through explicit \code{matrix} calls
are not necessarily picked up by that process; \code{cbind} requires a
second formation of a \code{ddmatrix}.
...
This is due to \code{diag-assign}
