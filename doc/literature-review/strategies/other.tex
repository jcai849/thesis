...
In operation, \pkg{Hadoop} splits files into blocks, then distributes them across nodes in a cluster (HDFS), where they are then processed by the node in parallel (MapReduce).
...
In recent years it has been overshadowed by \pkg{Spark}, due to the greater speed gains offered by \pkg{Spark} for many problem sets.
