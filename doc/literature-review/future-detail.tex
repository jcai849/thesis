...
\ccode{future-assign} with \ccode{plan-multiprocess}. This package
...
similarity to \pkg{future}. \R expressions can be captured in a \ccode{quote}, then evaluated in an
environment with \ccode{eval} at some point
in the future. Additionally, \ccode{substitute} substitutes any variables
...
\ccode{eval} functions and the like, but due
...
\begin{description}
    \item[\pkg{doFuture}]
        ...
        return \ccode{foreach} calls as futures. The multicore features enabled with
    ...
    \item[\pkg{future.apply}]
        ...
        \ccode{apply} procedures, with a future backend enabling parallel,
