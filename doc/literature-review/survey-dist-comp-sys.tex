\subsection{\pkg{Hadoop}}\label{sec:hadoop-1}
...
directly to the utilities using \file{.jar} files, ``streamed'' using any
...
\pkg{Spark}, due to the greater speed gains offered by \pkg{Spark}. The key reason
\pkg{Spark} is so much faster than \pkg{Hadoop} comes down to their different
...
by another Apache project, \pkg{Ignite}\cite{zheludkov2017high}.
...
HDFS is written in \proglang{Java} and \proglang{C}. It is described in more detail in \cite{shvachko2010hadoop}
...
\subsection{\pkg{Spark}}\label{sec:spark}
...
\pkg{Spark} is the libraries available that have pre-made functions for RDD's,
...
\pkg{Spark} has seen major use since it's introduction, with effectively all
