...
Furthermore, the name of the passed parameter is made available through call-by-need and is commonly made use of in R, such as the \ccode{deparse-substitute} employed for the implicit labelling of plot axes, among other uses.
...
As hinted at above, the expression is still capable of being accessed through the \ccode{substitute} function.
...
One of the fundamental procedures in \pkg{SNOW} is \ccode{cluster-call}\cite{tierney2018snow}.
...
However, early \lsr prototypes have been generating function calls through the \ccode{do-call} function, which constructs a call from a function and a list, then evaluates the call.
...
Beyond the above point regarding call creation,this leads to a missing capacity for procedures taking advantage of lazy evaluation, including the termination mechanism discussed in Section \cref{sec:evalinr} and illustrated in Listing \cref{lst:eval}.
...
Such a capacity is already inherent in R, with \ccode{delayed-assign} from the base package granting direct access to promise creation.
...
A drawback of the approach is evaluation of parameters prior to function application, but this may be sidestepped through the \proglang{C} API in a similar manner to the definition of \ccode{dots-length}\cite{abelson1996sicp:order}.

\src[Rout]{varnames}{Access to variable names given as formal parameters to a function call using x and y as defined in Listing \cref{lst:docall}}
