...
motivation behind \pkg{partools} with reference to \pkg{Hadoop} and \pkg{Spark}. Matloff
describes \pkg{partools} as more ``sensible'' for large data sets than \pkg{Hadoop}
and \pkg{Spark}, due to their difficulty of setup, abstract programming
paradigms, and the overhead caused by their fault tolerance. The
alternative approach favoured by \pkg{partools}, termed ``software alchemy'',
...
\ccode{calm}. Other functions in which it
...
explanatory variables, are catered for in \pkg{partools} through subsetting
...
\ccode{library} call, it won't be included in many other packages.
...
(\pkg{cdparcoord}) to plot coordinates for large datasets in parallel, and one
(\pkg{polyreg}) to form and evaluate polynomial regression models.
...
\pkg{bigmemory} matrices through \pkg{biganalytics}. The package is developed by
