...
Specifically, the entries in a node's finger table are the successors to the points relative to the node at increasing values of \mathfrom{tab-entries}
As such, the first element of a node's finger table, at \mathfrom{2-pow-0} points along from the node, will be that node's successor
...
Skip graph is a distributed version of the skip-list probabilistic data structure, with simple operations and impressive access costs\cite{aspnes2007skipgraph}.
The skip-list algorithm which underpins it is made use of by Redis in it's implementation of ordered sets.
