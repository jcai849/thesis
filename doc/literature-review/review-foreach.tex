...
function as in the registration for doParallel, \ccode{registerdoparallel}

The syntax of \pkg{foreach} consists of a \ccode{foreach}
 function call next to a \ccode{do} operator, and some expression to the
...
The \ccode{foreach} function can take other
...
In addition to \ccode{do}, other binary operators can be appended
or substituted. Parallel iteration is performed by simply replacing
\ccode{do} with \ccode{dopar}. Nested loops can be created by
inserting \ccode{foreach-nest} between main and nested \pkg{foreach} functions,
prior to the \ccode{do} call\cite{weston19:_nestin_loops}. The
last step to composition of \pkg{foreach} as capable of list comprehension is
the filtering function \ccode{when}, which filters iterables based
on some predicate to control evaluation.

\subsection{Implementation}\label{sec:implementation}

The mechanism of action in \pkg{foreach} is often forgotten in the face of the
atypical form of the standard syntax. Going one-by-one, the
\ccode{foreach} function returns an iterable
object, \ccode{do} and derivatives are binary functions operating
on the iterable object returned by
\ccode{foreach} on the left, and the
expression on the right; the rightmost expression is simply captured as
such in \ccode{do}. Thus, the main beast of burdern is the
\ccode{do} function, where the evaluation of the iteration takes
place.

In greater detail, \ccode{do} captures and creates environments,
enabling sequential evaluation. \ccode{dopar} captures the
...
\ccode{do} and \ccode{dopar}, after correct error checking,
send calls to \ccode{getdoseq} and
\ccode{getdopar} respectively, which return
...
used in turn with the \ccode{foreach}
...
coercible to one, but in other languages with \pkg{foreach} as part of the
core language, such as python (whose for loop is actually only a \pkg{foreach}
loop), collections can include sets, lists, and a variety of other
classes which have an \ccode[py]{iter} and
\ccode[py]{next} defined\cite{python2020iter}.
...
of the \pkg{foreach} package. The constraints are not insignificant
...
in \pkg{foreach} through the use of destructive reassignment within the passed
...
The key point of success in \pkg{foreach} is it's backend extensibility,
without which, \pkg{foreach} would lack any major advantages over a standard
for-loop.

Other parallel backends are enabled through specific functions made
available by the \pkg{foreach} package. The packages define their parallel
evaluation procedures with reference to the iterator and accumulator
methods from \pkg{foreach}.

Numerous backends exist, most notably:

\begin{description}
	\item[\pkg{doParallel}]
		the primary parallel backend for \pkg{foreach}, using the \pkg{parallel} package\cite{corporation19}.
	\item[\pkg{doRedis}]
	...
	\item[\pkg{doRNG}]
		...
		iterations, using L'Ecuyer's method; provides\ccode{dorng}\cite{gaujoux20}.
	\item[\pkg{doSNOW}]
...
For packages looking to provide any parallel capabilities, a \pkg{foreach} extension would certainly aid it's potential usefulness and visibility.
