...
The syntax of \pkg{foreach} consists of a \code{foreach} function call next to a \code{do} operator, and some expression to the right\cite{weston19:_using}.
...
In addition to \code{do}, other binary operators can be appended or substituted.
Parallel iteration is performed by simply replacing \code{do} with \code{dopar}.
Nested loops can be created by inserting \code{foreach-nest} between main and nested \pkg{foreach} functions, prior to the \code{do} call\cite{weston19:_nestin_loops}.
The last step to composition of \pkg{foreach} as capable of list comprehension is the filtering function \code{when}, which filters iterables based on some predicate to control evaluation.
...
Going one-by-one, the \code{foreach} function returns an iterable object, \code{do} and derivatives are binary functions operating on the iterable object returned by \code{foreach} on the left, and the expression on the right; the rightmost expression is simply captured as such in \code{do}.
Thus, the main beast of burden is the \code{do} function, where the evaluation of the iteration takes place.

In greater detail, \code{do} captures and creates environments, enabling sequential evaluation.
\code{dopar} captures the
...
In this case, the collection is an iterator object or an object coercible to one, but in other languages with \pkg{foreach} as part of the core language, such as python (whose for loop is actually only a \pkg{foreach} loop), collections can include sets, lists, and a variety of other classes which have an \code[py]{iter} and \code[py]{next} defined\cite{python2020iter}.
