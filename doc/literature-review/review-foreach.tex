...
The syntax of \pkg{foreach} consists of a \code{foreach}
 function call next to a \code{do} operator, and some expression to the
...
The \code{foreach} function can take other
...
In addition to \code{do}, other binary operators can be appended
or substituted. Parallel iteration is performed by simply replacing
\code{do} with \code{dopar}. Nested loops can be created by
inserting \code{foreach-nest} between main and nested \pkg{foreach} functions,
prior to the \code{do} call\cite{weston19:_nestin_loops}. The
last step to composition of \pkg{foreach} as capable of list comprehension is
the filtering function \code{when}, which filters iterables based
...
\code{foreach} function returns an iterable
object, \code{do} and derivatives are binary functions operating
on the iterable object returned by
\code{foreach} on the left, and the
expression on the right; the rightmost expression is simply captured as
such in \code{do}. Thus, the main beast of burdern is the
\code{do} function, where the evaluation of the iteration takes
place.

In greater detail, \code{do} captures and creates environments,
enabling sequential evaluation. \code{dopar} captures the
...
\code{do} and \code{dopar}, after correct error checking,
...
used in turn with the \code{foreach}
...
classes which have an \code[py]{iter} and
\code[py]{next} defined\cite{python2020iter}.
