...
\begin{displayquote}
	Support for the \pkg{foreach} looping construct. Foreach is an idiom that
	...
	require the evaluation of a function. Using \pkg{foreach} without side effects
...
The syntax of \pkg{foreach} consists of a
...
\end{minted}
	\caption{Standard \pkg{foreach} syntax}
	\label{lst:syntax}
\end{listing}
...
inserting \mintinline{r}{%:%} between main and nested \pkg{foreach} functions,
...
Due to the constraints imposed by a \pkg{foreach} construct, loop optimisation
...
are given in \cref{lst:serial}\cref{lst:serial-iter}.
...
\begin{description}
	\item[doParallel]
		the primary parallel backend for \pkg{foreach}, using the parallel
	...
	\item[doSNOW]
		provides an ad-hoc cluster backend, using the \pkg{SNOW}
...
For packages looking to provide any parallel capabilities, a \pkg{foreach}
