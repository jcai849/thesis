...
its embodiment in the representative objects offered by each layer. The
...
within its own memory. Over time, each subset may be pulled into the
...
accessed from the same location at differing time points is a question to
...
\code{read-csv}, or \code{dbconnect}, serving as just one particular
...
\code{max} or \code{sum}. For example, when looking for a
\code{max}, a chunk consisting of a billion integers is transformed
...
Here, we have the symbol \code{d1} specifying a reference to some
...
symbol \code{d1}. To interact with the integer vector directly, it may
...
the emerged vector to \code{l1}; that is, ``local''\code{1}.
...
Note that if run in a functional manner, the object \code{d2} is
entirely distinct and separate from \code{d1}. The mechanism of such a
transformation is described further in the following chapter.
Importantly, \code{d2} is also a chunk reference, with no emerges
having taken place. The chunk referred to may exist in a separate memory
space to the reference, just like \code{d1}. No specification is given
as to whether it necessarily exists in the same memory space as
\code{d1} however, only that both are remote to the reference.
...
data structure making up the chunks as pointed to. Operations over a
group of chunks may take a similar form to operations over a collection
of other objects, which are commonly referred to as \code{apply}
functions, both in \R{} and in the wider computer science world. A means of
operating over a group of chunks may be given in a \code{chunk-apply}
...
Take the earlier example of maximisation in \cref{lst:chunk-collection}, with new object \code{d3}
standing for a collection of chunks, being three chunk references, each
pointing to its own chunk of length one billion; The distributed object
is therefore a three billion length integer.

\src[Rout]{chunk-collection}{Collecting chunks into one object}

In \cref{lst:chunk-apply}, over each chunk, send the \code{max} function, to be performed
...
operation, \code{dmaxes}, consists of the remote results of the
operations, itself being a distributed object of three chunks. Each
chunk in \code{dmaxes} is the maximum value of each of the respective
\code{d3} chunks, as one scalar integer each, equivalently a
...
\code{lmaxes}, as in \cref{lst:chunk-summarised}

\src[Rout]{chunk-summarised}{Emergence of summarised chunks}

With \code{max} being a transitive operation, the max of
\code{lmaxes} may be taken in turn, with the operation being performed
as normal, locally, in \cref{lst:total-max}. This resulting value, assigned as \code{lmax}, is
equivalently the maximum of the entire distributed object \code{d3}.
...
may be read as a set out of chunks. The \code{chunk-apply} function
would sit as a valuable tool for the power-user layer, while
\code{max} may have some method defined for distributed objects that
uses \code{chunk-apply} in its implementation, and sits at the user
layer.
...
empirically, with the contribution made here to recognise its
significance.
...
\code{d4} -- consider this as sharing the same first chunk as
\code{d3}, with length \(1 \times 10^9\), but the second and third
...
the series above given as the variable \code{chunk-count}, we have
...
Treating the table as a vector, we may use \code{chunk-apply}, to run
a sample over each chunk, using \code{chunk-count} as the \(n\) for
...
as given in the original series, perhaps named \code{rearrange}, with
the series given as the variable \code{series}, we are left with a
