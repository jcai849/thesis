...
have it represented as a dataframe named \ccode{nyc-taxi}, with
columns to be accessed through the \ccode{dollar} operator and the name of
...
where \ccode{d1} refers to a group of three chunks treated as one
object:

\src[Rout]{chunk-object}{Group of chunks in a collection as a single object}

The \ccode{d} stands for ``\textbf{distributed}''; chunks must not
...
here given as variable \ccode{l1} in \cref{lst:emerged-chunks}. In this case, \ccode{l} stands for
