...
of a pointer in the \proglang{C} language takes the form in Listing \cref{lst:declare-int-pointer}:
...
of the symbol \ccode[c]{p}), which points to memory holding some integer.
Assuming that this piece of memory exists and has a value, the pointer
may be dereferenced using the \ccode[c]{star} operator as in listing \cref{lst:deref-c}

\src[c]{deref-c}{Dereference of a pointer in C}

And we have assigned the value stored in the address referred to by
\ccode{p}, to the variable \ccode{x}. The symbol \ccode{x} now stands
...
consider the distributed matrix class, \ccode{ddmatrix}, provided by
the pdDMAT package. Here is a more complex reference than a pointer,
where each \ccode{ddmatrix} reference may relate to an arbitrary number
of matrix blocks distributed across potentially disparate memory space.
Operations over \ccode{ddmatrix} objects are directly reflected as
...
the \ccode{ddmatrix} reference to the underlying matrix blocks:
...
\ccode{ddmatrix} object thereby refers both to the local block held by
...
other blocks, with the \ccode{ddmatrix} object being used to refer
solely to the local block. In more complex operations, such as singular
value decomposition, provided by \ccode{svd}, the reference given by
\ccode{ddmatrix} will access all other blocks - with all of this being
buried under a clean layer of abstraction. For most cases, the
referential nature of \ccode{ddmatrix} can be forgotten about, with the
...
the local block, through functions encoded with an \ccode{l} for
``local'' prefix. The example given by listing \cref{lst:dim-ldim} demonstrates the difference in
pbdR between \ccode{dim}, which gives the total dimensions of the
entire matrix, and \ccode{ldim}, which gives only the dimensions of the
block local to a particular node:

\src{dim-ldim}{The difference between dim and ldim in \pkg{pbdDMAT}}

In general, \ccode{submatrix} returns the local storage for the
\ccode{ddmatrix} local referent. We have with \pkg{pbdR} a converse of
...
\proglang{lisp} quotation and quasiquotation enabling explicit use/mention
interaction.
...
following elements as operands. For example, \ccode[lisp]{sum}
evaluates to \ccode[lisp]{6}, as summation. Values also self-evaluate;
\ccode[lisp]{6} evaluates to \ccode[lisp]{6}. The evaluation treats everything as
``in-use''. Such behaviour can be prevented through quotation
constructs. For example, \ccode[lisp]{quote-sum} evaluates to
\ccode[lisp]{sum}, which is just treated as just a list of symbols
(mention), ready for evaluation later on. Such a construct is so common,
it has the abbreviated form of the \ccode[lisp]{tick} - the
above example may have been equivalently quoted using
\ccode[lisp]{tick-sum} Quotation may also act in a
...
\ccode[lisp]{backquote} is used to quote, with \ccode[lisp]{quasiquote} used to
...
objects. Proximal to the \ccode[c]{star} operator for the dereferencing
operation in \proglang{C} is the \ccode[c]{address} operator for accessing the address of
...
in function names, is in \ccode{ddim}. This is similar to C's
\ccode[c]{directive}-directive pre-processing, though it requires an additional
modest burden on the developer. Alternatively, functions may be provided
to \ccode{materialise} and dematerialise the distributed object
