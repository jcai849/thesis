...
may be dereferenced using the \mintinline{r}{*} operator as in listing \cref{lst:deref-c}
...
finalize()
    \end{minted}
    \caption{Proxying of matrix operations in \pkg{pbdDMAT}}
...
\begin{listing}
    \begin{minted}{r}
        ...
        finalize()
    \end{minted}
    \caption{The difference between dim and ldim in \pkg{pbdDMAT}}
    \label{lst:dim-ldim}
\end{listing}

In general, \mintinline{r}{submatrix()} returns the local storage for the
\mintinline{r}{ddmatrix} local referent. We have with \pkg{pbdR} a converse of
...
evaluates to listing \cref{lst:unquoted}
