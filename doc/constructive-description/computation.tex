...
We may continue with the \ccode{max} example as raised in the preceding
section, with greater focus on the computational aspects. Starting
again, we have some chunk referenced through \ccode{d1} in \cref{lst:chunk-d1}:

\src[Rout]{chunk-d1}{Single chunk for manipulation}

We want to attain the maximum of this chunk. As given before, we may run
the \ccode{max} function over it, returning another chunk reference to
a chunk representing the \ccode{max} of \ccode{d1}, and emerge the
...
With respect to the \ccode{max} operation above, the first point means
that the chunk pointed to by \ccode{d1} must be located in the same
memory space as what the \ccode{max} operation has direct access to.
...
chunk references. We label it \ccode{chunk-call}, taking some function
...
implementation of remote \ccode{max} at a low level using
\ccode{chunk-call} in \cref{lst:chunk-call}.

\src[Rout]{chunk-call}{Directly calling a function on a chunk}

The use of \ccode{chunk-call} on a single chunk may be effectively
...
Concretely, consider vectors \ccode{v1}, \ccode{v2}, and \ccode{v3},
...
vector \ccode{i} and chunk \ccode{j}, as \ccode{dij}, so that the
third chunk of the first vector is referred to as \ccode{d13}. Then
each \(j\)th chunk of the vectors can be compared across with
\ccode{pmax}, using \ccode{chunk-call}, with the \cref{lst:call-along-chunks}
...
first vector \ccode{d1} may look something like the \cref{lst:distobj}:

\src[Rout]{distobj}{An example distributed object}

Here we may introduce a higher-level version of \ccode{chunk-call},
...
function \ccode{distobj-call}, and run it with \ccode{pmax}
over the distributed object vectors \ccode{d1}, \ccode{d2}, and
\ccode{d3} like \cref{lst:distobj-call}:
...
operation is to take the local argument \ccode{na-rm} A threshold
...
\ccode{na-rm} must be sent to the worker as part of the operation.
For the sake of consistency, it may be that \ccode{na-rm} is sent over
...
had a very obvious interpretation, where \ccode{na-rm} is sent as-is to
wherever the chunk referred to by \ccode{d1} is located. Even in a
situation like the \ccode{distobj-call}, run with a
\ccode{max} over a single distributed object, it is obvious that
\ccode{na-rm} should be sent to all workers operating on each chunk of
...
the greater object. For instance, the \ccode{pmax} between a scalar and
...
Figure \cref{fig:scalar-index} shows precisely this example. Here, \ccode{x} is a
chunk containing a single integer, \ccode{1}, and \ccode{y} is a
distributed object composed of three chunks, each holding a section from
\ccode{1up}. In all chunk computations with \ccode{max-x-y},
\ccode{x} is to be recycled to engage as an argument with all chunk
operations involving \ccode{y}. Conversely, if \ccode{y} is to be
indexed by \ccode{x}, it should not be recycled, and each chunk
belonging to \ccode{y} should receive different transformations of
\ccode{x}. Specifically, the first chunk of \ccode{y} should receive
\ccode{x} as-is, because with \ccode{x} as \ccode{1}, it selects the
...
other chunks. Were a \ccode{1} to be indexed from each of the other
...
Here, the variable \ccode{a} is of particular interest, as it requires
